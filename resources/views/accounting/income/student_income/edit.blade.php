                ...
                <div class="card-body p-0">
                    <div class="modal-dialog modal-xl">
                        <div class="modal-content">
                            <form action="{{route('student-income.update', $record->id)}}" method="post">
                                @method('put')
                                @csrf
                                        ...
                                        <div class="col-md-4">
                                            <div class="mb-3">
                                                ...
                                                <x-input-select wireModel="project_id" name="project_id"
                                                                :records="$projects"
                                                                targetColumn="title"
                                                                selected="{{$record->income->project_id}}">
                                                </x-input-select>
                                            </div>
                                        </div>
                                        <div class="col-md-4">
                                                                ...
                                                                @foreach($courses as $course)
                                                                    <optgroup label="{{$course['title']}}">
                                                                        @foreach($course['children'] as $childCourse)
                                                                            <option
                                                                                value="{{$childCourse['id']}}" {{$detail->course_id == $childCourse['id'] ? 'selected' : ''}}>{{$childCourse['title']}}</option>
                                                                        @endforeach
                                                                    </optgroup>
                                                                @endforeach
                                                            </select>
                                ...
                                <div class="modal-footer">
                                    <button type="submit" class="btn font-size-16 btn-success"> update </button>
                                </div>
                            </form>
                        </div>
                    </div>
